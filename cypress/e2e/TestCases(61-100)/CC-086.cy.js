import { login, abrirParametros, abrirFactura, abrirTipoDocumentoIdentidad, closeModal } from './funciones';

describe('Verificar selección automática del tipo de documento configurado como "Defecto"', () => {
  ...
  let defectoAbreviatura = ''; // Variable para guardar la abreviatura del tipo de documento "Defecto"
  ...
  it('Validar que el tipo de documento "Defecto" se preseleccione automáticamente al facturar', () => {
    abrirTipoDocumentoIdentidad(); // Abrir Tipos de Documento de Identidad

    // Paso 1: Iterar sobre los switches y capturar la abreviatura del tipo de documento configurado como "Defecto"
    ...
    cy.get(`${tableRowSelector}`).each(($row) => {
      ...
      if (!defectoEncontrado) {
        cy.log('No hay ningún tipo de documento configurado como "Defecto".');
        throw new Error('Ningún tipo de documento está configurado como "Defecto".');
      }
    });
