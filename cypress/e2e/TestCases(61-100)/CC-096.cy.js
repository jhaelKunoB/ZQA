import { login, abrirParametros, abrirFactura, abrirUnidadesMedidaSIM, closeModal } from './funciones';

describe('Verificar selección automática del Unidades de medida SIM configurado como "Defecto"', () => {
  ...
  let defectoAbreviatura = ''; // Variable para guardar la abreviatura del Unidades de medida SIM "Defecto"
  ...
  it('Validar que el Unidades de medida SIM "Defecto" se preseleccione automáticamente al facturar', () => {
    abrirUnidadesMedidaSIM(); // Abrir Unidades de medida SIM

    // Paso 1: Iterar sobre los switches y capturar la abreviatura del Unidades de medida SIM configurado como "Defecto"
    ...
    cy.get(`${tableRowSelector}`).each(($row) => {
      ...
      if (!defectoEncontrado) {
        cy.log('No hay ningún Unidades de medida SIM configurado como "Defecto".');
        throw new Error('Ningún Unidades de medida SIM está configurado como "Defecto".');
      }
    });
