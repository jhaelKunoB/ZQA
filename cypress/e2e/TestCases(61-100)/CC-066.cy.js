import { login, abrirParametros, abrirFactura, abrirMotivoFacturaManual, closeModal } from './funciones';

describe('Verificar selección automática del Motivo Factura Manual configurado como "Defecto"', () => {
  ...
  let defectoAbreviatura = ''; // Variable para guardar la abreviatura del Motivo Factura Manual "Defecto"
  ...
  it('Validar que el Motivo Factura Manual "Defecto" se preseleccione automáticamente al facturar', () => {
    abrirMotivoFacturaManual(); // Abrir Motivo Factura Manual

    // Paso 1: Iterar sobre los switches y capturar la abreviatura del Motivo Factura Manual configurado como "Defecto"
    ...
    cy.get(`${tableRowSelector}`).each(($row) => {
      ...
      if (!defectoEncontrado) {
        cy.log('No hay ningún Motivo Factura Manual configurado como "Defecto".');
        throw new Error('Ningún Motivo Factura Manual está configurado como "Defecto".');
      }
    });
